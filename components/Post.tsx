    ...
    return (
        <Card>
            <div className='flex gap-3 w-full items-start relative'>
                <Avatar />
                <div>
                    <p><Link href={''} className='font-semibold'>Jon Doe</Link> shared an <Link href={''} className='text-blue-500'>album</Link></p>
                ...
                <div className={`absolute bg-white top-6 -right-6 p-3 shadow-md shadow-gray-300 rounded-sm border border-gray-100 ${dropDown ? 'opacity-100': 'opacity-0'} transition-all duration-150`}>
                    <div ref={menu} className={`${dropDown ? 'opactity-100' : 'opacity-0'}`}>
                        ...
                        <button className='flex items-center gap-2 py-2 px-2 text-red-500/40 hover:text-red-500 transition-colors duration-200'>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
